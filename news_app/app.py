...
def get_news_data(option, classification, num_news):
    ...
    cur.execute(
        f"SELECT COUNT(*) FROM {table_name} WHERE classification = ?", (classification,)
    )
...
def get_news_data_ml(option, ml_classification):
    ...
    cur.execute(
        f"SELECT COUNT(*) FROM {table_name} WHERE ml_classification = ?",
        (ml_classification,),
    )
    count = cur.fetchone()[0]
...
news_option = st.sidebar.radio("Transport:", ("🚢 Maritime News", "✈️ Air News"))

# Drop-down menu for choosing news location
news_loc = st.sidebar.radio("Location:", ("🌐 Global", "🌎 South America"))

# Drop-down menu for choosing news categories
news_cat = st.sidebar.selectbox(
    "Category:",
    (
        "Unclassified or Neutral",
        "Regulations",
        "Issues",
        "Global Trade",
        "Port Operations",
...
if st.session_state.show_text:
    st.markdown(
        "<h4 style='margin-bottom:0px; color: #FFFFCC;'>👈 Select your news of interest...</h4>",
...
if st.sidebar.button("Display News!", on_click=hide_text):
    ...
    if len(news_items) == 0:
        ...
        st.markdown(
            '<p class="notification">No news found today!</p>', unsafe_allow_html=True
        )
    else:
        for news_title, news_content, summary, link in news_items:
            ...
            with st.container():
                ...
                with col1:
                    ...
                    st.markdown(
                        f'<p class="subheader-font"><a href="{link}" target="_blank">{news_title}</a></p>',
                        unsafe_allow_html=True,
                    )
                    with st.expander("Read More"):
                        st.markdown(
                            f'<div class="quote">{news_content[:1300]}...</div>',
                            unsafe_allow_html=True,
                        )
                with col2:
                    st.markdown(
                        f'<div class="caption-code">🤖 {summary}</div>',
                        unsafe_allow_html=True,
                    )
...
# App section
st.markdown(
    '<p class="centered-subheader">AI Recommended News</p>', unsafe_allow_html=True
)
# Insert custom HTML to create a horizontal line separator
st.markdown('<hr style="border:1px solid red">', unsafe_allow_html=True)
...
with st.container():
    ...
    news_title, news_content, summary, link = get_news_data_ml(
        random_option, ml_classification="risk"
    )
    ...
    with col2:
        ...
        st.markdown(
            f'<div class="caption-code">🤖 {summary}</div>', unsafe_allow_html=True
        )
...
with st.container():
    ...
    random_option = random.choice([1, 2])
    news_title, news_content, summary, link = get_news_data_ml(
        random_option, ml_classification="opportunities"
    )
...
with st.container():
    ...
    random_option = random.choice([1, 2])
    news_title, news_content, summary, link = get_news_data_ml(
        random_option, ml_classification="neither"
    )
...
# App section
st.markdown(
    '<p class="subheader-premium">🔓 PREMIUM Example</p>', unsafe_allow_html=True
)
# Insert custom HTML to create a horizontal line separator
...
with st.container():
    ...
    with col1:
        ...
        # Header for the news title
        st.markdown(
            f'<p class="subheader-font"><a href="{link_project}" target="_blank">Tensions in the Red Sea</a></p>',
            unsafe_allow_html=True,
        )
        # Use an expander for displaying the news content
        with st.expander("Read More"):
            st.markdown(
                f'<div class="quote">{text_example}</div>', unsafe_allow_html=True
            )
    with col2:
        # Summary of the news title
        st.markdown(
            f'<div class="quote-test">🤖 AI-Powered Summary:</div>',
            unsafe_allow_html=True,
        )
        st.write(f"")
